...
void CSequenceMain::Init_LoadTray(int nPNo, int nSNo)
{
	...
	else 
	{
		for (int i = 0; i < nCmCnt; i++) 
		{
			tempX = i / nTrayY;
			tempY = i % nTrayY;			
			gData.InfoLoadTray[tempY][tempX] = 9;
		}
	}
...
#ifdef AKC
		gData.InfoUnloadTray[y][x] = 2; //AKC
#endif
	
#ifndef AKC // CHS_KS, CHS_W, CHS_Z
...
BOOL CSequenceMain::TrayPicker_Run()
{
	...
	case 1:		// AVI Tray Check.
		if (m_bUnloadLotEnd || gData.bCycleStop || !m_pEquipData->bUseInlineMode) {
			m_strLog.Format("m_nTrayPickCase,%d",m_nTrayPickCase); g_objLogFile.Save_MCCLog(m_strLog);
...
BOOL CSequenceMain::LoadStage1_Run()
{
	...
	if (!m_pEquipData->bUseInlineMode) {
		...
		if (((m_bLotLoadEnable[0] == TRUE 
			&& ((m_pEquipData->bUseDoorLock && m_pDX13->iDoor01Unlock) || !m_pEquipData->bUseDoorLock) 
			&& (nLs1WorkPort == 1 && m_nLoadStage1Case > 3 && m_nLoadStage1Case < 10)) 
			|| (m_bLotLoadEnable[1] == TRUE 
			&& ((m_pEquipData->bUseDoorLock && m_pDX13->iDoor02Unlock) || !m_pEquipData->bUseDoorLock) 
			&& (nLs1WorkPort == 2 && m_nLoadStage1Case > 3 && m_nLoadStage1Case < 10))))
			return TRUE;
	}
	...
	case 1:		// Load Tray, Port 확인.
		if (g_objCommon.Check_Position(AX_LOAD_STAGE1_Z, 1)) {	// 안전 확인
			...
			{
				m_strLog.Format("m_nLoadStage1Case,%d",m_nLoadStage1Case); g_objLogFile.Save_MCCLog(m_strLog);
				nLs1WorkPort = 0;
				...
				if (m_pEquipData->bUseInlineMode) {
					...
					} else if ((m_pDX00->iLoadPort1LowCheck || m_pDX01->iLoadPort2LowCheck) && !gData.bCycleStop)
					{
						...
						if ((nLs1WorkPort == 0 || nLs1WorkPort == 2) && m_pDX00->iLoadPort1LowCheck && (gData.nCmUseCount[0] > 0)) 
						{ nLs1WorkPort = 1; nLs1AviPort = 1; }
						else if ((nLs1WorkPort == 0 || nLs1WorkPort == 1) && m_pDX01->iLoadPort2LowCheck && (gData.nCmUseCount[1] > 0)) 
						{ nLs1WorkPort = 2; nLs1AviPort = 2; }
	...
	case 2:		// Tray X Move to Load Position
		...
		if (gData.nLoadTrayCount[nLs1AviPort-1] <= gData.nTrayUseCount[nLs1AviPort-1] && !gData.bCycleStop) 
		{
			...
			if (nLs1WorkPort == 1 && !g_objCommon.Check_Position(AX_LOAD_STAGE1_X, 0)) 
				g_objCommon.Move_Position(AX_LOAD_STAGE1_X, 0);	// Port1
			if (nLs1WorkPort == 2 && !g_objCommon.Check_Position(AX_LOAD_STAGE1_X, 1)) 
...
BOOL CSequenceMain::LoadStage2_Run()
{
	...
	if (!m_pEquipData->bUseInlineMode) {
		...
		if (((m_bLotLoadEnable[0] == TRUE 
			&& ((m_pEquipData->bUseDoorLock && m_pDX13->iDoor01Unlock) || !m_pEquipData->bUseDoorLock)
			&& (nLs2WorkPort == 1 && m_nLoadStage2Case > 3 && m_nLoadStage2Case < 10)) 
			|| (m_bLotLoadEnable[1] == TRUE 
			&& ((m_pEquipData->bUseDoorLock && m_pDX13->iDoor02Unlock) || !m_pEquipData->bUseDoorLock) 
			&& (nLs2WorkPort == 2 && m_nLoadStage2Case > 3 && m_nLoadStage2Case < 10)))
		) 
		return TRUE;
	}
			...
			else
			{
				if (m_pEquipData->bUseInlineMode)
				{
						...
						(m_nTrayPickCase < 12 || m_nTrayPickCase > 15))
					{
						...
						m_strLog.Format("m_nLoadStage2Case,%d",m_nLoadStage2Case); g_objLogFile.Save_MCCLog(m_strLog);
				...
				else 
				{
					...
					else if ((m_pDX00->iLoadPort1LowCheck || m_pDX01->iLoadPort2LowCheck) && !gData.bCycleStop) 
					{
						// 맨처음 시작할때 2개 모두 적재 되어있으면 1번부터 가져간다.
						// 작업할 포트에 트레이 유무와 입력수량확인.
						if ((nLs2WorkPort == 0 || nLs2WorkPort == 2) && m_pDX00->iLoadPort1LowCheck && (gData.nCmUseCount[0] > 0))
						{ 
							nLs2WorkPort = 1; nLs2AviPort = 1; 
						}
						else if ((nLs2WorkPort == 0 || nLs2WorkPort == 1) && m_pDX01->iLoadPort2LowCheck && (gData.nCmUseCount[1] > 0)) 
						{
							nLs2WorkPort = 2; nLs2AviPort = 2; 
						}
...
BOOL CSequenceMain::LoadPicker_Run()
{
	...
	case 1:		// Tray가 Work 위치에 오면 시작
		...
		{	// Index에 모듈이 있을때 Tray가 안넘어오면 Index는 돌아가게 해준다.
			...
			{
					if (!Check_IndexEmpty(-1) && m_nVisCmAlignCase == 0) 
					{
	...
	case 2:		// X/Y Move to Pickup Position
		if (gData.nPickerUseCnt < 6 && m_pEquipData->bSortDirRight) {
				...
				(nLpWorkTray == 2 && g_objCommon.Check_Position(AX_LOAD_STAGE2_X, 2))))
			{
				if (Select_LoadTrayPos(nLpTrayPosX, nLpTrayPosY))
				{
					...
					m_strLog.Format("m_nLoadPickCase,%d",m_nLoadPickCase); g_objLogFile.Save_MCCLog(m_strLog);
				}
			}
		} 
		else 
		{			
			...
			if (!g_objCommon.Get_LoadPickerUp() || !g_objCommon.Check_Position(AX_LOAD_PICKER_Z, 0)) break;
			// 			
	...
	case 3:		// Z Axis Move to Tray Down
		if (gData.nPickerUseCnt < 6 && m_pEquipData->bSortDirRight) 
		{
			...
			{			
				...
				m_nLoadPickCase++; m_tLoadPickLoop.Set_LoopTime(10000);
				m_strLog.Format("m_nLoadPickCase,%d,",m_nLoadPickCase); 
		...
		{	// Picker 6ea or Load Tray 잔량정렬 아래방향
				...
				g_objCommon.Check_Position(AX_LOAD_PICKER_P, 0))
			{
				...
				g_objCommon.Move_Position(AX_LOAD_PICKER_Z, nLpWorkTray);
				m_nLoadPickCase++; m_tLoadPickLoop.Set_LoopTime(10000);
				m_strLog.Format("m_nLoadPickCase,%d,[Start]LoadPickerDownMulti To Get CM On Tray",m_nLoadPickCase);
				g_objLogFile.Save_MCCLog(m_strLog);
			}
		}
		break;
	...
	case 7:	// Picker Check
		if (g_objCommon.Get_LoadPickerUp() && g_objCommon.Get_InfoLoadPickerCmCheck()
			&& g_objCommon.Check_Position(AX_LOAD_PICKER_Z, 0))
		{
...
//					gData.nCmJigNo[nPNo][nTNo][nCNo][INDEX_LOAD_NO]	 = nIdxNo;	// Index No
	...
	case 18:	// Picker Up
		if (g_objCommon.Get_LoadPickerOpen()) 
		{
			...
			m_nLoadPickCase++; m_tLoadPickLoop.Set_LoopTime(15000);
			m_strLog.Format("m_nLoadPickCase,%d",m_nLoadPickCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
	case 19:	// return
...
BOOL CSequenceMain::MainIndex_Run()
{
	...
	case 0:
		...
		else if (m_nLoadPickCase > 4 && m_nLoadPickCase <= 15) 
		{	// Loading
			m_nMainIndexCase++; m_tMainIndexLoop.Set_LoopTime(5000);
			m_strLog.Format("m_nMainIndexCase,%d",m_nMainIndexCase); 
			g_objLogFile.Save_MCCLog(m_strLog);
		}
		return TRUE;
	...
	case 1:	// Index Align1 Out & Vacuum Pad Up
		...
		m_strLog.Format("m_nMainIndexCase,%d,[Start]Index Align1 Out & Vacuum Pad Up",m_nMainIndexCase);
		g_objLogFile.Save_MCCLog(m_strLog);
		break;
	...
	case 2:	// Check Index Align1 Out & Vacuum Pad Up
		if (!m_pDX11->iIndexLoadAlignIn && m_pDX11->iIndexLoadAlignOut && m_pDX11->iIndexLoadVacUp && !m_pDX11->iIndexLoadVacDown) {
			...
			m_strLog.Format("m_nMainIndexCase,%d,[Done] Index Align1 Out & Vacuum Pad Up",m_nMainIndexCase); 
			g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
	...
	case 5:	// Wait for Load Picker Done
		...
		{	// LoadPicker Up 동작 후
			...
			m_strLog.Format("m_nMainIndexCase,%d,[Start]LoadVacuum On",m_nMainIndexCase); 
			g_objLogFile.Save_MCCLog(m_strLog);
		}
		return TRUE;
	...
	case 6:	// Retry (Align Out) or Vacuum Off
		if (m_pDX11->iIndexLoadAlignIn && !m_pDX11->iIndexLoadAlignOut) {
			if (g_objCommon.Get_InfoIndexLoadVacuumOn(0))
			{
				...
				m_strLog.Format("m_nMainIndexCase,%d,[Done]LoadVacOn [Start]LoadVacOff",m_nMainIndexCase); 
				g_objLogFile.Save_MCCLog(m_strLog);
			} 
			else
			{
				...
				{	// Retry 1회
					...
					m_strLog.Format("m_nMainIndexCase,%d,[Start]Align Out",m_nMainIndexCase); 
					g_objLogFile.Save_MCCLog(m_strLog);
				}
			}
		}
	...
	case 7:	// Align1 In (Retry)
		if (!m_pDX11->iIndexLoadAlignIn && m_pDX11->iIndexLoadAlignOut) 
		{
			...
			g_objAJinAXL.Write_Output(11);
			m_nMainIndexCase = 6; m_tMainIndexLoop.Set_LoopTime(5000);
			m_strLog.Format("m_nMainIndexCase,%d,[Done]AlignOut [Start]AlignIn",m_nMainIndexCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
	case 8:	// Vacuum Pad Down
		if (g_objCommon.Get_IndexLoadVacuumOff(0))
		{
			...
			m_strLog.Format("m_nMainIndexCase,%d,[Start]LoadVacDown",m_nMainIndexCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
	case 9:	// Check Vacuum Pad Down
		...
		{
			m_nMainIndexCase = 10; m_tMainIndexLoop.Set_LoopTime(5000);
			m_strLog.Format("m_nMainIndexCase,%d",m_nMainIndexCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
		...
		if (gData.IndexDone[0] && gData.IndexDone[1] && gData.IndexDone[2]) {
				...
				!m_pDX11->iIndexLoadVacUp && m_pDX11->iIndexLoadVacDown && !m_pDX11->iIndexAssyVacUp && m_pDX11->iIndexAssyVacDown) 
			{
					 ...
					 (g_objCommon.Check_Position(AX_TRANS_STAGE_Z, 0) || g_objCommon.Check_Position(AX_TRANS_STAGE_Z, 2))))) 
				{
						...
						m_strLog.Format("m_nMainIndexCase,%d,[Start]Index Rotation",m_nMainIndexCase); 
						g_objLogFile.Save_MCCLog(m_strLog);
				}
			}
		}
		return TRUE;
	case 11:	// Check R Move Done and Index End
		if (g_objAJinAXL.Is_MoveDone(AX_INDEX_R, m_pMoveData->dIndexR[0])) 
		{
			Set_IndexEnd();
			m_nMainIndexCase = 0; m_tMainIndexLoop.Set_LoopTime(5000);
			m_strLog.Format("m_nMainIndexCase,%d,[Done]Index Rotation",m_nMainIndexCase); 
			g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
			...
			!m_pDX11->iIndexLoadVacUp && m_pDX11->iIndexLoadVacDown && !m_pDX11->iIndexAssyVacUp && m_pDX11->iIndexAssyVacDown) 
		{
			if ((g_objAJinAXL.Get_Position(AX_LOAD_PICKER_Y) <= 200.0 || (g_objAJinAXL.Get_Position(AX_LOAD_PICKER_Y) > 200.0 && g_objCommon.Get_LoadPickerUp())) &&
				...
				(g_objCommon.Check_Position(AX_TRANS_STAGE_X, 0) || g_objCommon.Check_Position(AX_TRANS_STAGE_X, 2))))) {
					...
					m_strLog.Format("m_nMainIndexCase,%d,[Start]Home Search",m_nMainIndexCase); g_objLogFile.Save_MCCLog(m_strLog);
			}
		}
		break;
	case 22:		// Check R Home Done
		...
		{
			m_nMainIndexCase = 0;
			m_strLog.Format("m_nMainIndexCase,%d,[Done]Home Search",m_nMainIndexCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
	}
	...
	case 1:		// X Move Inspect Position
		if (g_objCommon.Check_Position(AX_VISION_CM_ALIGN_X, 0)) 
		{
			if (m_pEquipData->bUseVisionCmAlign) {
				...
				m_strLog.Format("m_nVisCmAlignCase,%d",m_nVisCmAlignCase); g_objLogFile.Save_MCCLog(m_strLog);
	...
	case 1:		// X Move Inspect Position
		if (g_objCommon.Check_Position(AX_VISION_CAP_ALIGN_Y, 0))
		{
			if (m_pEquipData->bUseVisionCapAlign) 
			{
				...
				m_strLog.Format("m_nVisCapAlignCase,%d",m_nVisCapAlignCase); g_objLogFile.Save_MCCLog(m_strLog);
...
BOOL CSequenceMain::CapStage1_Run()
{
	...
	case 1:		// 안전확인, Z Move to Move Up
		if (m_bUnloadLotEnd) 
		{
			...
			m_strLog.Format("m_nCapStage1Case,%d",m_nCapStage1Case); g_objLogFile.Save_MCCLog(m_strLog);
		}
...
BOOL CSequenceMain::CapStage2_Run()
{
	...
	case 1:		// 안전확인, Z Move to Move Up
		if (m_bUnloadLotEnd) 
		{
			...
			m_strLog.Format("m_nCapStage2Case,%d",m_nCapStage2Case); g_objLogFile.Save_MCCLog(m_strLog);
		}
...
BOOL CSequenceMain::CapPicker_Run()
{
	...
	case 0:
		if (m_nCapStage1Case == 20 || m_nCapStage2Case == 20)
		{
			m_nCapPickCase++;
			m_strLog.Format("m_nCapPickCase,%d",m_nCapPickCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		return TRUE;
	case 1:		// 작업 시작
	...
	case 2:		// Stage X Picker Y Move to Cap Pickup Position
		if (g_objCommon.Get_CapPickerUp(0) && g_objCommon.Check_Position(AX_CAP_PICKER_Z, 0)
			&& g_objAJinAXL.Is_Done(AX_CAP_PICKER_Y)) {
				if (m_nCapStage1Case != 20 && m_nCapStage2Case != 20) return TRUE;	// 인터락
				...
				if (Select_CapPickerPos(nCpStart, nCpPickCnt) && Select_CapTrayPos(nCpPosX, nCpPosY, nCpTrayCnt)) {
	...
	case 3:		// Z Move to Tray Down
		if (((nCpWorkTray == 1 && g_objAJinAXL.Is_MoveDone(AX_CAP_STAGE1_X, dCpX)) ||
			...
			g_objAJinAXL.Is_MoveDone(AX_CAP_PICKER_Y, dCpY) && g_objCommon.Check_Position(AX_CAP_PICKER_P, 0))
		{			
		...
		{			
			...
			else 
			{
				if (Check_CapTrayEmpty()) 
				{
					...
					int nPickCnt = 0;
					if (Select_CapPickerPos(nCpStart, nPickCnt)) 
					{
						if (m_pEquipData->bCapPickUpMulti) 
						{
							nCpTrayCnt = m_pEquipData->nCapTrayX - nCpPosX;
	...
	case 16:
		...
		break;
	case 17:	// Buffer Go
		if (g_objCommon.Check_Position(AX_CAP_PICKER_Z, 0) && g_objCommon.Get_CapPickerUp(0) && m_pDX09->iCapCleanDown && !m_pDX09->iCapCleanUp) {
			if (m_nCapBufferCase == 0) { m_nCapBufferCase = 1; m_tCapBufferLoop.Set_LoopTime(10000); }
	...
	case 20:	// return
		...
		break;
	case 21:	// Check
		if (g_objAJinAXL.Is_MoveDone(AX_CAP_PICKER_Y, dCpY) && g_objCommon.Check_Position(AX_CAP_PICKER_P, 0) 
			&& g_objCommon.Get_InfoCapPickerNormal())
		{
			m_nCapPickCase = 1; m_tCapPickLoop.Set_LoopTime(10000);			
		}
		break;
...
// 			g_objCommon.Move_Position(AX_CAP_BUFFER_STAGE_Y, 1);
			...
			m_strLog.Format("m_nCapBufferCase,%d",m_nCapBufferCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
...
BOOL CSequenceMain::AssyPicker_Run()
{
	...
	case 2:		// Z Move to Buffer Down, Picker Vac On
			...
			g_objCommon.Get_AssyPickerUp(0) && m_pDX08->iAssyPickerTiltUp && !m_pDX08->iAssyPickerTiltDown) 
		{
			...
			m_nAssyPickCase++; m_tAssyPickLoop.Set_LoopTime(5000);
			m_strLog.Format("m_nAssyPickCase,%d",m_nAssyPickCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		break;
	...
	case 6:		// Position Check
		if (g_objCommon.Check_Position(AX_ASSY_PICKER_X, 1) 
			&& g_objCommon.Check_Position(AX_ASSY_PICKER_Y, 1) 
			&& g_objCommon.Check_Position(AX_ASSY_PICKER_Z, 2)) 
		{
	...
	case 15:	// Index Check
		if (m_nMainIndexCase <= 10 && !gData.IndexDone[1]) {
			if (Check_IndexEmpty(1) && !gData.IndexDone[1])
			{ 
	...
	case 28:	// Vac Off Check & Air Off, X/Y Axis Move to Tilt Check Position
		if (g_objCommon.Check_Position(AX_ASSY_PICKER_Z, 5) 
			&& g_objCommon.Get_AssyPickerUp(0) 
			&& g_objCommon.Get_InfoAssyPickerVacOff(0)) 
		{
	...
	case 29:	// Tilt Check Cylinder Down
		if (g_objCommon.Check_Position(AX_ASSY_PICKER_Z, 5) 
			&& g_objCommon.Check_Position(AX_ASSY_PICKER_X, 6) 
			&& g_objCommon.Check_Position(AX_ASSY_PICKER_Y, 6) &&
...
// 					g_objAJinAXL.Write_Output(8);
	...
	case 70:	// Assy Picker Cylinder Down
		if (g_objCommon.Get_AssyPickerUp(0) 
			&& g_objCommon.Check_Position(AX_ASSY_PICKER_Z, 0) 
			&& g_objAJinAXL.Is_MoveDone(AX_CAP_BUFFER_STAGE_Y, dApY)) 
		{
			nApJobNo++;
...
BOOL CSequenceMain::TransStage_Run()
{
	...
	case 0:		// Index Check
		if (m_nMainIndexCase <= 10 && !gData.IndexDone[2]) 
		{
			...
			m_strLog.Format("m_nTransStageCase,%d",m_nTransStageCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
		return TRUE;
...
BOOL CSequenceMain::UnloadPicker_Run()
{
	...
	case 0:		// Trans Stage Check
		if (m_nTransStageCase == 10) 
		{
			...
			m_strLog.Format("m_nUnloadPickCase,%d",m_nUnloadPickCase); g_objLogFile.Save_MCCLog(m_strLog);
		}
...
// 			if (m_nTransStageCase == 10) m_nTransStageCase = 11;
	...
	case 5:		// Check Picker Up, Move to Unload Stage1 Position
		if (g_objCommon.Get_InfoUnloadPickerVacOn(1) 
			&& g_objCommon.Get_UnloadPickerUp(0) && g_objCommon.Check_Position(AX_UNLOAD_PICKER_Z, 0))
		{
	...
	case 11:	// Move to Unload Tray, Tray Position Check
			...
			g_objAJinAXL.Is_MoveDone(AX_UNLOAD_PICKER_X, dUpX))
		{
			...
			{
				if (Select_UnloadTrayPos(nUpPosX, nUpPosY, nUpPosCnt))
				{					
	...
	case 15:	// Position Check, Unload Tray Full Check
		if ( g_objCommon.Get_InfoUnloadPickerVacOff(0)) 
		{			
...
BOOL CSequenceMain::UnloadStage1_Run()
{
	...
	case 1:		// 안전확인
			...
			m_pDX03->iUnloadPort1SlideClose && m_pDX03->iUnlaodPort1LowCheck)
		{
			...
			m_strLog.Format("m_nUnloadStage1Case,%d",m_nUnloadStage1Case); g_objLogFile.Save_MCCLog(m_strLog);
		} 
		else 
		{
	...
	case 8:		// Tray Check, Stage Master In
		...
		{
			if (!m_tUnloadStage1Loop.Waiting_Time(200)) break;
	...
	case 10:	// 안전 확인 
		if (m_nUnloadStage2Case > 23) 
		{
			if (!m_tUnloadStage1Loop.Waiting_Time(100)) return TRUE;
			m_nUnloadStage1Case = 12; m_tUnloadStage1Loop.Set_LoopTime(5000);
			m_strLog.Format("m_nUnloadStage1Case,%d",m_nUnloadStage1Case); g_objLogFile.Save_MCCLog(m_strLog);
		}
		return TRUE;

	case 12:	// Y Move to Work Position
		if (m_pDX05->iUnloadStage1Exist && g_objCommon.Check_Position(AX_UNLOAD_STAGE1_Y, 1)
			&& g_objAJinAXL.Is_Done(AX_UNLOAD_STAGE1_Z)) 
		{						
	...
	case 15:	// Ship 정보 확인 및 MES 자재 등록.
		...
		{
			if (m_pEquipData->bUseMes && m_pEquipData->bUseMesShipReg) 
			{	// MES Ship 자재등록 사용
	...
	case 21:	// 안전 확인
		if (m_nUnloadStage2Case <= 10 || m_nUnloadStage2Case > 52) 
		{
			...
			{				
				...
				m_nUnloadStage1Case = 23; m_tUnloadStage1Loop.Set_LoopTime(5000);
				m_strLog.Format("m_nUnloadStage1Case,%d",m_nUnloadStage1Case); g_objLogFile.Save_MCCLog(m_strLog);
			}
		}
		return TRUE;
...
BOOL CSequenceMain::UnloadStage2_Run()
{
	...
	case 1:		// 안전확인
			...
			m_pDX03->iUnloadPort1SlideClose && m_pDX03->iUnlaodPort1LowCheck)
		{
			...
			m_strLog.Format("m_nUnloadStage2Case,%d",m_nUnloadStage2Case); g_objLogFile.Save_MCCLog(m_strLog);
	...
	case 10:	// 안전 확인 
		if (m_nUnloadStage1Case > 23) 
		{
			if (!m_tUnloadStage2Loop.Waiting_Time(100)) return TRUE;
			m_nUnloadStage2Case = 12; m_tUnloadStage2Loop.Set_LoopTime(5000);
			m_strLog.Format("m_nUnloadStage2Case,%d",m_nUnloadStage2Case); g_objLogFile.Save_MCCLog(m_strLog);
		}
		return TRUE;

	case 12:	// Y Move to Work Position
		if (g_objCommon.Check_Position(AX_UNLOAD_STAGE2_Y, 1) && m_pDX05->iUnloadStage2Exist
			&& g_objAJinAXL.Is_Done(AX_UNLOAD_STAGE2_Z)) 
		{				
	...
	case 21:	// 안전 확인
		if (m_nUnloadStage1Case <= 10 || m_nUnloadStage1Case > 52) 
		{
			...
			{				
				...
				m_nUnloadStage2Case = 23; m_tUnloadStage2Loop.Set_LoopTime(5000);
				m_strLog.Format("m_nUnloadStage2Case,%d",m_nUnloadStage2Case); g_objLogFile.Save_MCCLog(m_strLog);
			}
		}
		return TRUE;
