...
BOOL CWorkDlg::OnInitDialog()
{
	...
	m_bLotIdInput[0] = FALSE;
	m_bLotIdInput[1] = FALSE;
...
LRESULT CWorkDlg::OnUpdateBarcode(WPARAM wParam, LPARAM lParam)
{
	...
	else if (m_bLotIdInput[0]) nType = 0;
	else if (m_bLotIdInput[1]) nType = 0;
	else						return 0;

	int nCapCnt = 0;
	if( nType > 0)
	{
		nCapCnt = Check_CapShipLotID(nType, sData);
	...
	if (m_bLotIdInput[0]) {
		gData.sLotID[0] = sData;
		m_stcLotIdR.SetWindowText(gData.sLotID[0]);	
	}

	if (m_bLotIdInput[1]) {
		gData.sLotID[1] = sData;
		m_stcLotIdS.SetWindowText(gData.sLotID[1]);	
	}
