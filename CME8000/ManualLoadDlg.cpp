...
void CManualLoadDlg::OnShowWindow(BOOL bShow, UINT nStatus) 
{
	...
	m_btnLoadPickerIo[ 0].EnableWindow(bOn);	// Picker 1 Up
	m_btnLoadPickerIo[ 6].EnableWindow(bOn);	// Picker 1 Down
	m_btnLoadPickerIo[12].EnableWindow(bOn);	// Picker 1 Open
	m_btnLoadPickerIo[18].EnableWindow(bOn);	// Picker 1 Close
...
#ifdef CHS_KS
	if (nIndex == 10 || nIndex == 25) pDY06->oLoadPickerDown05 = TRUE;
#endif
...
#ifdef CHS_KS
	if (nIndex == 22 || nIndex == 27) pDY06->oLoadPickerGrip05 = TRUE;
#endif	
